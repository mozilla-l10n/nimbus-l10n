...
def print_toml_file(toml_data):
    # Format the TOML file to minimize differences with the format normally used
...
def issue_exists(repo, issue_number):
    # Verify if an issue number is available in the repository
...
def main():
    ...
    # already went through this pipeline.
    ...
    # Check if an issue already exists, create a new one if not
    gh_repo = args.repo
